---
title: "Template"
author: "Studentnames and studentnumbers here"
date: "`r Sys.Date()`"
output: pdf_document
---

# Set-up your environment

```{r package_install, include=FALSE}
install.packages("tidyverse")
install.packages("yaml")
install.packages("rmarkdown")
install.packages("WDI")
install.packages("eurostat")
```

```{r packages}
require(tidyverse)
require(rmarkdown)
require(yaml)
require(WDI)
require(eurostat)
```

# Title Page


LE Vi Trang Raghoebar Monisha Isabel Lust Ermias Gebremichael Pritish Dewan Pritish Dewan Noel Tripathi Caelin Rohiemon

Tutorial 6 Group 2

Simon Loop

# Part 1 - Identify a Social Problem

Use APA referencing throughout your document. [Here's a link to some explanation.](https://www.mendeley.com/guides/apa-citation-guide/)

## 1.1 Describe the Social Problem

Include the following:

- Why is this relevant?

- ...

# Part 2 - Data Sourcing

## 2.1 Load in the data

```{r loading_data and retrieve only data of European countries}

options(timeout = 600)
# Eurostat 
ai_raw <- get_eurostat("isoc_eb_ai", time_format = "num")

# Filter out EU countries, fix the 2 misaligned EU codes and drop aggregates
eu_codes <- ai_raw %>%
  distinct(geo) %>%
  mutate(iso2c = recode(geo, "EL" = "GR", "UK" = "GB")) %>%
  filter(nchar(iso2c) == 2, iso2c != "EA") %>%
  pull(iso2c)

# World Bank: download ONCE, then reuse the saved copy
if (file.exists("wb_raw.rds")) {
  wb_raw <- readRDS("wb_raw.rds")
} else {
  wb_raw <- WDI(
    country   = eu_codes,
    indicator = c(total_unemp = "SL.UEM.TOTL.ZS",
                  youth_unemp = "SL.UEM.1524.ZS"),
    start = 1991, end = 2024, extra = TRUE
  )
  saveRDS(wb_raw, "wb_raw.rds")
}
```

## 2.2 Provide a short summary of the dataset(s)

```{r}
head(ai_raw) 
str(ai_raw)
summary(ai_raw)
names(ai_raw)

head(wb_raw)
str(wb_raw)
summary(wb_raw)
names(wb_raw)

```

## 2.3 Describe the type of variables included

Think of things like:

- Do the variables contain health information or SES information?

- Have they been measured by interviewing individuals or is the data coming from administrative sources?

|     | Col2 | Col3 | Col4 |
|-----|------|------|------|
|     |      |      |      |
|     |      |      |      |
|     |      |      |      |
|     |      |      |      |

: Table 1

Variables Description: AI Adoption & Unemployment Rate
  Health vs. SES Information:* No direct health data is included. Neither variable measures physical or mental health. However, both have indirect links: AI adoption can be filtered to analyze the healthcare sector (NACE Sector Q), and unemployment is widely used as a macro-determinant of population stress.  Yes, both are core socioeconomic status (SES) indicators.* AI adoption tracks firm-level SES (corporate wealth and resource scale), while the unemployment rate tracks national macro-SES (financial vulnerability and living standards).
  
 Data Source (Survey vs. Administrative): Both variables are entirely survey-driven.Neither comes from automated government registries. The AI data is self-reported by company representatives via corporate ICT surveys. The unemployment data consists of modeled ILO estimates compiled from national household Labor Force Surveys. Both rely on sampling and self-reporting rather than institutional registers.
 
 Geographic & Temporal Scope: The dataset is strictly limited to European countries over the annual timeline of 2021 to 2025. This specific window captures the exact intersection of the post-COVID labor market recovery and the rapid corporate adoption boom following the launch of generative AI.
 
*For the sake of this example, I will continue with the assignment...*

# Part 3 - Quantifying

## 3.1 Data cleaning

Say we want to include only larger distances (above 2) in our dataset, we can filter for this.

```{r data_cleaning}

wb_cleaned <- wb_raw %>%
  filter(year == 2021 | year == 2022 | year == 2023 | year == 2024)

ai_clean <- ai_raw %>%
  filter(indic_is == "E_AI_TANY")
  





```

Please use a separate 'R block' of code for each type of cleaning. So, e.g. one for missing values, a new one for removing unnecessary variables etc.

## 3.2 Generate necessary variables

Variable 1: Exposure gap between youth and the total population

```{r gen_var1}

wb_clean <- wb_clean %>%
  mutate(youth_ratio = youth_unemp / total_unemp)
```

Variable 2: AI intensity usage across EU countries

```{r gen_var2} 

combined <- wb_clean %>%
  left_join(ai_clean, by = c("iso2c" = "geo", "year" = "TIME_PERIOD")) %>%
mutate(ai_group = case_when(
  values < 10 ~ "Low",
  values < 30 ~ "Medium",
  TRUE ~ "High"
))
names(wb_clean)

```

Table with new variables

```{r}
combined %>%
  group_by(ai_group) %>%
  summarise(
    n = n(),
    mean_total_unemp = mean(total_unemp, na.rm = TRUE),
    mean_youth_unemp = mean(youth_unemp, na.rm = TRUE),
    mean_youth_ratio = mean(youth_ratio, na.rm = TRUE)
  )
```

## 3.3 Visualize temporal variation
```{r}
# 1. Average unemployment rate per year
yearly <- wb_clean %>%
  group_by(year) %>%
  summarise(
    mean_total_unemp = mean(total_unemp, na.rm = TRUE),
    mean_youth_unemp = mean(youth_unemp, na.rm = TRUE)
  )

# 2. Plot the 'yearly' table
ggplot(yearly, aes(x = year)) +
  geom_line(aes(y = mean_total_unemp, color = "Total")) +
  geom_line(aes(y = mean_youth_unemp, color = "Youth (15-24)")) +
  labs(title = "EU unemployment over time: youth vs total",
       x = "Year", y = "Unemployment (% of labour force)",
       color = "Group") +
  theme_minimal()
```


ai_group - bins the continuous values into Low, Medium, and High, so we can compare youth unemployment outcomes across different levels of AI adotion.

## 3.4 Visualize spatial variation

```{r visualise_map}
ggplot ("eu countries", aes(x=ai_intensity, 
                            y= youth_unemployment,
                            size= total_unemployment,
                            label=country)) +
  geom_point(alpha=0.7) + 
  geom_text_repel(size=3)+
  scale_size_continuous(name="total unemployment (%)") +
  
  labs(x="AI intensity", 
       y="youth unemployment(%)", 
       title= "AI intensity and unemployment in EU countries") + theme_minimal()
                 
```

Here you provide a description of why the plot above is relevant to your specific social problem.

## 3.5 Visualize sub-population variation

Youth unemployment in high- vs low-AI-adoption countries (2024).

```{r visualise_subpopulations}
combined %>%
  filter(year == 2024, !is.na(ai_group)) %>%
  mutate(ai_group = factor(ai_group, levels = c("Low", "Medium", "High"))) %>%
  ggplot(aes(x = ai_group, y = youth_unemp)) +
  geom_boxplot(na.rm = TRUE, fill = "violet", color = "navy", outlier.color = "darkorange1", varwidth = TRUE) +
  labs(title = "Youth unemployment by level of AI adoption (2024)",
       x = "AI adoption group", y = "Youth unemployment (%)") +
  theme_minimal()
```

Template:
```{r visualise_subpopulations}
dataset$inmetro <- dataset$inmetro %>% as.factor()
# Boxplot of poverty rate by state using the 'midwest' dataset
ggplot(dataset, aes(x = inmetro, y = percadultpoverty)) +
  geom_boxplot() +
  labs(
    title = "Distribution of Poverty Rates by Metropolitan status (Midwest counties)",
    x = "Metropolitan Area",
    y = "Poverty Rate of Adults (%)"
  ) +
  theme_minimal() +
  theme(
    legend.position = "right"
  )
```

Here you provide a description of why the plot above is relevant to your specific social problem.

## 3.6 Event analysis

Analyze the relationship between two variables.

```{r analysis}

```

Here you provide a description of why the plot above is relevant to your specific social problem.

# Part 4 - Discussion

## 4.1 Discuss your findings

# Part 5 - Reproducibility

## 5.1 Github repository link

Provide the link to your PUBLIC repository here: ...

## 5.2 Reference list

Use APA referencing throughout your document.
